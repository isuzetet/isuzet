...
export interface StrategyConfig {
  ...
  // Phase 10: Cold chain
  coldChainTempRanges: Record<string, { min: number; max: number }>;
  ...
  noShowDriverWindowMin: number; // Grace period before driver no-show flag
  noShowOrdererWindowMin: number; // Orderer no-show window
  noShowDriverFeeCents: number; // Fee charged to driver for no-show
  noShowOrdererFeeCents: number; // Fee charged to orderer for no-show
  noShowRecipientFeeCents: number; // Fee for recipient absent at delivery
  recipientAbsentGracePeriodMin: number; // Grace period before auto-release
  deliveryPhotoGpsRadiusM: number; // GPS validation radius for photo delivery
  autoReleaseEscrowHours: number; // Auto-release escrow after this many hours (already exists)
  checkpointNormalDelayMin: number; // Normal delay threshold
  checkpointEscalationThresholdMin: number; // Escalation threshold for detention
  hosAdvisoryHours: number; // Advisory notification threshold
  hosBlockHours: number; // Hard block threshold
...
export const DEFAULT_CONFIG: Omit<StrategyConfig, 'id' | 'versionName'> = {
  wdmWeights: {
    proximity: 0.11,
    trust: 0.16,
    onTimeRate: 0.18,
...
export async function getConfig(): Promise<StrategyConfig> {
  ...
  try {
    ...
    const record = await prisma.strategyConfig.findFirst({
      where: { isActive: true },
    ...
    const merged: StrategyConfig = {
      ...
      ...DEFAULT_CONFIG,
      ...(record.configJson as Partial<StrategyConfig>),
    };
